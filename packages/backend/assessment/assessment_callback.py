...
def _find_report_html_key(session_id):
    """S3에서 해당 세션의 HTML 레포트 키를 찾는다."""
    prefix = f'assessments/{session_id}/html/'
    try:
        ...
        print(f"Error listing S3 objects for session {session_id}: {str(e)}")
    return None
...
def handle_codebuild_event(event, context):
    ...
    if build_status == 'SUCCEEDED':
        # HTML 레포트 키 탐색
        report_key = _find_report_html_key(session_id)
        ...
        if report_key:
            ...
            expr_values[':report_key'] = report_key
        ...
        _record_assessment_event(
            ...
            details={'reportS3Key': report_key or '', 'buildStatus': build_status},
            ttl=ttl,
        )

        print(f"Session {session_id} assessment completed. Report: {report_key}")
