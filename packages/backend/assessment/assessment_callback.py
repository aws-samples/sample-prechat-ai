...
def handle_reporting_codebuild_event(event, context):
    """Reporting CodeBuild 완료 이벤트를 처리한다.

    SUCCEEDED → dashboardS3Key를 세션에 저장
    세션 ID는 ATHENA_SUMMARY_CSV 경로에서 추출한다.
    경로 형식: s3://{bucket}/assessments/{sessionId}/reports/*.csv
    """
    detail = event.get('detail', {})
    build_status = detail.get('build-status', '')
    project_name = detail.get('project-name', '')
    ...
    if build_status != 'SUCCEEDED':
        print(f"Reporting build {build_status} for {project_name}, skipping")
        return lambda_response(200, {'message': f'Skipped: {build_status}'})

    # ATHENA_SUMMARY_CSV 환경변수에서 세션 ID 추출
    env_vars = (
        detail.get('additional-information', {})
        .get('environment', {})
        .get('environment-variables', [])
    ...
    csv_path = ''
    for var in env_vars:
        if var.get('name') == 'ATHENA_SUMMARY_CSV':
            csv_path = var.get('value', '')
            break

    if not csv_path:
        print(f"No ATHENA_SUMMARY_CSV in Reporting CodeBuild event")
        return lambda_response(400, {'error': 'Missing CSV path'})

    # s3://{bucket}/assessments/{sessionId}/reports/*.csv → sessionId 추출
    parts = csv_path.replace('s3://', '').split('/')
    session_id = ''
    for i, part in enumerate(parts):
        if part == 'assessments' and i + 1 < len(parts):
            session_id = parts[i + 1]
            break

    if not session_id:
        print(f"Could not extract session ID from: {csv_path}")
        return lambda_response(400, {'error': 'Could not extract session ID'})
    ...
    # 대시보드 HTML 키 탐색
    dashboard_key = _find_report_key(session_id, 'reports', '.html')

    if not dashboard_key:
        print(f"Dashboard HTML not found for session {session_id}")
        return lambda_response(200, {'message': 'No dashboard found'})
    ...
    sessions_table.update_item(
        Key={'PK': f'SESSION#{session_id}', 'SK': 'METADATA'},
        UpdateExpression='SET dashboardS3Key = :dk, updatedAt = :ts',
        ExpressionAttributeValues={
            ':dk': dashboard_key,
            ':ts': timestamp,
        },
    ...
    _record_assessment_event(
        session_id, 'dashboard_generated',
        details={'dashboardS3Key': dashboard_key},
        ttl=session.get('ttl'),
    )

    print(f"Session {session_id} dashboard saved: {dashboard_key}")
    return lambda_response(200, {'message': 'Dashboard key saved'})
