...
|---------|------|--------|-------------|------|
| Consultation Agent | 고객 사전 상담 | STM (AgentCore Memory, 30일) | `stream` (SSE) | AgentConfig로 동적 구성 (retrieve, current_time, render_form, aws_docs_mcp, http_request, extract_a2t_log) |
...
│   ├── config_parser.py          # AgentConfig → Agent 초기화 파라미터
│   ├── tool_registry.py          # 지원 도구 레지스트리
...
Consultation Agent는 AgentConfig에서 `aws_docs_mcp` 도구를 선택하면 [AWS Documentation MCP Server](https://github.com/awslabs/mcp)가 활성화됩니다. 에이전트가 고객 상담 중 AWS 공식 문서를 실시간으로 검색하여 정확한 정보를 제공할 수 있습니다.
...
Summary Agent는 Pydantic 모델로 타입 안전한 응답을 반환합니다.
...
```json
{
  ...
  "config": { "locale": "ko", "model_id": "모델 ID (선택)" }
}
```
