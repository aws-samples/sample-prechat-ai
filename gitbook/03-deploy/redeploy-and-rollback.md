...
| 변경 대상 | 재배포 명령 |
|----------|-----------|
| 프론트엔드 (`packages/web-app/`) | `./deploy-website.sh dev default ap-northeast-2 mte-prechat-workshop` |
| 백엔드 도메인 디렉토리 (`session/`, `campaign/` 등) | `sam build && sam deploy` |
| 공통 코드 (`shared/`) | `sam build && sam deploy` (Layer 전체 업데이트) |
| `template.yaml` | `sam build && sam deploy` |
| 에이전트 (`consultation-agent/`, `summary-agent/`) | `./packages/strands-agents/deploy-agents.sh` |
| 전체 | `./deploy-full.sh` |
...
[이메일 계정 가입하기](../04-admin/create-admin-account.md)로 이동해 실제 기능을 사용합니다.
