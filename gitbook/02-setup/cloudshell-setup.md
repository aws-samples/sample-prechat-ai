...
```bash
# 기본 도구 버전 확인
aws --version && sam --version && node --version && python3 --version

# npm으로 의존성 설치 (Yarn 대신 npm 사용)
npm install -g npm@latest

# uv 설치 (에이전트 배포 도구에 필요)
...
CloudShell 홈 디렉토리(`/home/cloudshell-user`)는 세션 간 유지됩니다. `pip install --user`로 설치한 Python 패키지도 홈에 저장되므로 재설치가 필요하지 않습니다.