...
### CloudShell 세션이 끊기거나 로그아웃됨

CloudShell은 유휴 20~30분이면 세션이 종료됩니다. 콘솔에서 로그아웃해도 마찬가지입니다. 이때 홈 디렉토리(`~`) 바깥에 클론한 레포지토리는 삭제될 수 있습니다.

**영향 범위**

| 항목 | 상태 |
|------|------|
| CloudFormation 스택 (배포된 인프라) | 그대로 유지됨 |
| SSM 파라미터, Cognito 사용자 | 그대로 유지됨 |
| `uv`, `sam` 등 설치한 도구 (`~/.local/bin`) | 그대로 유지됨 |
| 클론한 레포지토리 (`~/sample-prechat-ai`) | 삭제될 수 있음 |

**해결**

레포지토리만 다시 클론하면 작업을 재개할 수 있습니다. 이미 배포된 스택에는 영향이 없습니다.
...
{% hint style="info" %}
레포를 홈 디렉토리(`~`) 안에 클론하면 세션이 끊겨도 파일이 보존됩니다. 워크샵 초기 설정에서 `~/sample-prechat-ai`에 클론하는 이유입니다.
{% endhint %}

### CloudShell 디스크 부족

```bash
rm -rf ~/.aws-sam /home/workshop/sample-prechat-ai/node_modules
du -sh ~/* | sort -h
```
