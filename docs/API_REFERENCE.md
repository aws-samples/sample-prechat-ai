...
## Assessment Domain (Public - PIN 인증 필요)

SHIP(Security Health Improvement Program) 보안 점검 워크플로우 API입니다.

| Method | Path | Description |
|--------|------|-------------|
| POST | `/api/sessions/{sessionId}/assessment/legal-consent` | 법적 규약 동의 제출 |
| POST | `/api/sessions/{sessionId}/assessment/role` | AssumeRole ARN 제출 (스캔 시작) |
| GET | `/api/sessions/{sessionId}/assessment/status` | Assessment 상태 조회 |
| GET | `/api/sessions/{sessionId}/assessment/report-url` | 레포트 다운로드 URL 생성 |

### POST /api/sessions/{sessionId}/assessment/legal-consent

법적 규약 동의를 제출합니다. 멱등성 보장 — 이미 동의 상태면 200 반환.

### POST /api/sessions/{sessionId}/assessment/role

IAM Role ARN을 제출하고 Prowler 보안 스캔을 시작합니다 (CodeBuild).

```json
{
  "roleArn": "arn:aws:iam::123456789012:role/ProwlerScanRole"
}
```

## Admin - Assessment (Cognito Auth)

| Method | Path | Description |
|--------|------|-------------|
| GET | `/api/admin/sessions/{sessionId}/assessment` | Assessment 상태 조회 (Admin) |
| GET | `/api/admin/sessions/{sessionId}/assessment/report-url` | 레포트 다운로드 URL (Admin) |

## Event-Driven - Assessment

| Trigger | Description | Handler |
|---------|-------------|---------|
| EventBridge (CodeBuild) | CodeBuild 상태 변경 이벤트 처리 | `assessment_callback.handle_codebuild_event` |
...
- `agentRole`: prechat | summary | planning | ship
