...
## Slide 5: 가장 먼저 챙겨야 할 것

> **Before adopting any AI tool, align the context to your team first.**
>
> — AI 도구를 도입하기 전에, 먼저 당신 팀에 맞는 컨텍스트를 구성하세요.

> 📝 발표자 노트:
> 이 슬라이드는 후킹 메시지 하나만 크게 띄우는 장표입니다. 커뮤니티의 성공 사례들은 훌륭한 참고 자료입니다. 하지만 고객 입장에서 가장 먼저 챙겨야 할 건, 그 사례들이 공유하는 컨텍스트 설계 원리를 이해하고 우리 팀에 맞게 구성하는 것입니다. 남의 컨텍스트를 그대로 가져오는 게 아니라, 우리 팀의 기술 스택, 도메인, 워크플로우에 맞춰 재구성해야 진짜 효과가 납니다. Kiro는 이 과정을 IDE 레벨에서 구조화해줍니다.

---

## Slide 6: Why Kiro?

**당신이 어떤 팀이든, SW Development LifeCycle에서 거치는 핵심 공통 패턴을 IDE 자체에서 제공합니다.**

Kiro가 제안하는 6가지 메커니즘:

| 메커니즘 | 역할 |
|---------|------|
| **Spec** | 요구사항 → 설계 → 작업의 체계적 개발 라이프사이클 |
| **Steering** | 팀의 기술 스택, 도메인 지식, 코드 규칙을 AI에 상시 주입 |
| **Hooks** | IDE 이벤트에 반응하는 자동화 트리거 (보안 체크, 품질 검증, 배포) |
| **Skills** | 재사용 가능한 AI 행동 패턴 (보안 리뷰, 코드 리뷰, TDD 등) |
| **MCP** | 외부 도구/서비스와의 표준 통신 프로토콜 (AWS Docs, DB 등) |
| **Power** | MCP 서버 + 문서 + 워크플로우 가이드를 패키징한 확장 단위 |

이 메커니즘들을 통해, **적은 노력의 컨텍스트 구성**으로 당신 팀에 align하는 개발 환경을 구성할 수 있습니다.

> 📝 발표자 노트:
> 다른 도구들은 "여기 빈 md 파일이 있으니 알아서 채우세요"라는 접근입니다. Kiro는 6가지 메커니즘으로 개발 라이프사이클 전체를 커버합니다. Spec으로 뭘 만들지 정의하고, Steering으로 팀 컨텍스트를 주입하고, Hooks로 반복 작업을 자동화하고, Skills로 전문 역할을 위임하고, MCP로 외부 서비스를 연결하고, Power로 이 모든 걸 패키징합니다. 시작점이 명확하니 진입 장벽이 낮고, 팀마다 필요한 만큼만 구성하면 됩니다.
...
## Slide 14: 횡단 관심사 자동화 — Session Wrap-up Hook
...
- 에이전트 대화가 끝날 때마다 세션 대화 내용을 정리
- TODO와 Git Commit 메시지를 제안
- Next Action Item을 놓치지 않을 수 있어서 **상당한 도움이 됨**

> 📝 발표자 노트:
> 이게 생각보다 정말 유용했습니다. 작업하다 보면 "아 이것도 해야 하는데" 하고 까먹는 경우가 많잖아요. 에이전트 대화가 끝날 때마다 자동으로 세션 내용을 정리해주고, 커밋 메시지와 TODO를 제안해주니까 흐름이 끊기지 않았습니다. 단순한 Hook 하나인데 개발 습관 자체를 개선해주는 효과가 있었습니다.
...
3. Kiro는 SW Development LifeCycle의 핵심 공통 패턴을 IDE에서 제공하여, **Spec · Steering · Hooks · Skills · MCP · Power** 메커니즘을 통해 적은 노력의 컨텍스트 구성으로 당신 팀에 align하는 개발 환경을 구성할 수 있게 해줍니다

**Spec + Steering + Hooks + Skills + MCP + Power = 팀에 Align된 AI 개발 환경**

> 📝 발표자 노트:
> 마무리입니다. 오늘 전달하고 싶은 건 하나입니다. "AI 도구를 잘 쓰는 건 프롬프트를 잘 쓰는 게 아니라, 컨텍스트를 잘 설계하는 것이다." Kiro는 6가지 메커니즘으로 그 설계를 쉽게 만들어줍니다. 감사합니다.
